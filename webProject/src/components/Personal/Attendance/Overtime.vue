<template>
	<div id="late-early-main">
		...
		<div class="recruit-user-btn">
			<el-button type="primary" @click="addApplyBtnEvent()" size="small" icon="el-icon-plus">加班申请</el-button>
		</div>
		<el-table
			:data="tableData"
			style="width: 100%"
			...
			<el-table-column prop="orId" label="编号" width="75"></el-table-column>
			<el-table-column prop="employeeId" label="员工编号" width="100"></el-table-column>
			<el-table-column prop="startDate" label="加班日期" width="200"></el-table-column>
			<el-table-column prop="applyHours" label="小时数" width="100"></el-table-column>
		...
		<el-dialog
		  ...
		  :before-close="handleClose">
			<OvertimeForm ref="OvertimeFormData" :isAdd="isAddOperation"></OvertimeForm>
		  <template #footer>
			<span>
...
import OvertimeForm from '@/components/Personal/Attendance/OvertimeForm.vue';
import {GetForParams,submitDataForPost,getDataForNullParams} from '@/network/RequestApi.js';
export default{
	name:'',
	components:{
		SearchCondition,Pagination,OvertimeForm
	},
	data(){
		return{
	...
	methods:{
		...
		//确定加班事件
		entryApplyBtnEvent(){
			let data = this.$refs.OvertimeFormData.formData;
			data.startDateTemp = this.myFormatDate(data.DateTemp);
		...
		addApplyBtnEvent(){
			...
			else this.$refs.OvertimeFormData.resetData();
		...
		submitExamineInfo(data){
			submitDataForPost("/overtime-record/examine",data).then(res=>{
				for(let i=0;i<this.tableData.length;i++){
					if(this.tableData[i].orId == data.orId){
		...
		submitApplyInfo(data){
			submitDataForPost("/overtime-record/insert",data).then(res=>{
		...
		async getOneRecord(value){
			...
			GetForParams("/overtime-record/one",params).then(res=>{
		...
		async getInitInfo(){
			...
			GetForParams("/overtime-record/all",null).then(res=>{
