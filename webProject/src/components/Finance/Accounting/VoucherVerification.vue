<template>
	<div class="office-disk-main">
		...
		<div style="margin-bottom: 10px;">
			<el-button type="primary" size="medium" @click="addApplyBtnEvent" icon="el-icon-plus">批量审核</el-button>
...
export default{
	...
	methods:{
		...
		addApplyFailedBtnEvent(){
			...
			this.$confirm('一旦提交无法修改，是否继续审核凭证?','提示',{
				...
				for(let i=0;i<this.currentSelectedItem.length;i++){
					...
					temp.reviewId = this.$store.getters.employeeId;
					temp.reviewName = 'reviewName';
					temp.state = '审核未通过';
					data.push(temp);
				}
		...
		submitRecordData(data){
			submitDataForPost("/voucher/review",data).then(res=>{
				this.clearTableSelection();
				for(let i=0;i<data.length;i++){
					for(let j=0;j<this.tableData.length;j++){
						if(this.tableData[j].bvId == data[i].bvId){
							this.tableData[j].reviewId = data[i].reviewId;
							this.tableData[j].reviewName = data[i].reviewName;
							this.tableData[j].state = data[i].state;
							this.tableData[j].reviewDate = res.data.data;
							break;
						}
					}
				}
		...
		//页面打开时拉取所有信息
		async getInitInfo(){
			getDataForNullParams("/voucher/all-review").then(res=>{
