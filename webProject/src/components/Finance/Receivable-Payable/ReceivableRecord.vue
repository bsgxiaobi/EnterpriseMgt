<template>
	<div class="office-disk-main">
		...
		<el-table
			...
			<el-table-column fixed prop="sarId" label="单据编号" width="250">
				<template #header>
					<label>单据编号</label>
					<el-button type="text" size="mini" @click="addApplyBtnEvent" icon="el-icon-plus">添加应收单</el-button>
				</template>
			</el-table-column>
			<el-table-column prop="customerInfo.name" label="往来客户" width="200"></el-table-column>
			...
			<el-table-column prop="received" label="已收" width="100"></el-table-column>
		...
		<el-divider content-position="left">收款单:</el-divider>
		...
		<el-table id="contacts-table"
			...
			:cell-style="contactsTableRowClass"
			height="300">
			<el-table-column fixed prop="srId" label="单据编号" width="200">
				<template #header>
					<label>单据编号</label>
			  ...
			  :before-close="handleClose">
				<el-form :inline="true" v-model="formData"  class="demo-form-inline">
					...
					<el-form-item label="客户:">
						<el-input v-model="formData.cid" placeholder="客户"></el-input>
					</el-form-item>
					<el-form-item label="应收:">
						<el-input v-model="formData.receivable" placeholder="应收金额"></el-input>
					</el-form-item>
			  ...
			  <el-form :inline="true" v-model="sonFormData"  class="demo-form-inline">
			  	<el-form-item label="收款类型:">
					<el-select v-model="sonFormData.type" placeholder="请选择">
					  <el-option label="销售货物收入" value="销售货物收入"></el-option>
					  <el-option label="预收款" value="预收款"></el-option>
					  <el-option label="冲减应收款" value="冲减应收款"></el-option>
...
let formData={cid:null,receivable:null,abstracts:'',agent:null,agentName:''};
	...
	methods:{
		...
		entrySonApplyBtnEvent(){
			this.$confirm('单据保存后将无法修改，确定输入无误','提示',{
				...
				data.sarId = this.currentOperationId;
				data.agent = 2100003;
		...
		customerRowClickEvent(row){
			this.currentOperationId = row.sarId;
			this.getAllSonInfo(row.sarId);
		},
		//点击新增应付(收)按钮事件
		addApplyBtnEvent(){
			let formDataTemp={cid:null,receivable:null,abstracts:'',agent:null,agentName:''};
		...
		submitSonRecordData(data){
			...
			submitDataForPost("/single-receipt/insert",data).then(res=>{
				...
				for(let i=0;i<this.tableData.length;i++){
					if(this.tableData[i].sarId == data.sarId){
						this.tableData[i].received = Number(this.tableData[i].received) + Number(data.total);
						break;
					}
				}
				this.$message({
		...
		submitRecordData(data){
			submitDataForPost("/single-ar/insert",data).then(res=>{
				let result = res.data.data;
		...
		async getAllSonInfo(id){
			let params = {};
			params.id = id;
			GetForParams("/single-receipt/all",params).then(res=>{
		...
		//页面打开时拉取所有信息
		getInitInfo(){
			getDataForNullParams("/single-ar/all").then(res=>{
